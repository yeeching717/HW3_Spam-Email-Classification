...
def main():
    # Initialize data processor and classifier with recommended settings
    # Recommended: ngram_range=(1,2), min_df=2, sublinear_tf=True
    data_processor = DataProcessor(ngram_range=(1,2), min_df=2, sublinear_tf=True)
    # Use tuned params: C=10.0, penalty='l1', solver='liblinear', class_weight='balanced'
    classifier = SpamClassifier(penalty='l1', C=10.0, solver='liblinear', class_weight='balanced')
    ...
    processed_path = os.path.join('datasets', 'processed', 'sms_spam_clean.csv')
    data_processor.save_processed(df, processed_path)
